...
describe("Patchnode chat registration", () => {
  it("exposes the read tool without an action gate", async () => {
    ...
    expect(tools.map((tool) => tool.name)).toContain("fn_patchnode_read");
  });
  ...
  it("omits the tool when no task store exists", async () => {
    ...
    expect(tools.map((tool) => tool.name)).not.toContain("fn_patchnode_read");
  });
});
