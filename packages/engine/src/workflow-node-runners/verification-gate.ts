...
import { UNATTRIBUTED_MUTATION_CONTEXT, type RunMutationContext } from "@fusion/core";

export type DeterministicVerificationGateDeps = {
  store: TaskStore;
  getRunContextFor?: (taskId: string) => EngineRunContext | undefined;
  /*
  FNXC:Identity 2026-08-29-23:50 (U18/KTD2):
  The deterministic verification seam now requires a total mutation context. Optional here because
  a caller with no live run (the gate's own tests) has no actor to name; it degrades to the
  unattributed marker so the debt stays greppable rather than reading as real attribution.
  */
  ...
  const result = await runVerification(
    {
      ...
      runContextFor: deps.runContextFor ?? (() => UNATTRIBUTED_MUTATION_CONTEXT),
    },
    task,
    worktreePath,
    settings,
