    ...
    try {
      ...
      */
      if (live.status === "failed" && live.error?.startsWith("NO REVIEWABLE CONTENT:")) {
        ...
        if (afterSettle.status !== "failed" || afterSettle.error !== parkedError) {
          await deps.store.updateTask(task.id, { status: "failed", error: parkedError }, deps.runContextFor(task.id));
        }
        deps.clearPausedAborted(task.id);
        deps.activeWorktrees.delete(task.id);
        ...
        await deps.store.logEntry(task.id, emptyParkHonored, undefined, deps.runContextFor(task.id));
        await deps.persistTokenUsage(task.id);
        return;
      }
      /*
      ...
      */
      if (deps.userCanceledTaskIds.has(task.id)) {
        ...
        await deps.store.logEntry(task.id, cancellationExit, undefined, deps.runContextFor(task.id));
        await deps.persistTokenUsage(task.id);
        return;
      }
      /*
