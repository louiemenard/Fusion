...
export async function retryPlanningLifecycleLockTransportFailure(
  deps: Pick<RunImplementationDeps, "store" | "getRunContextFor" | "runContextFor" | "markGraphExecuteSelfRequeued">,
  task: Task,
  errorMessage: string,
  ...
  await deps.store.logEntry(task.id, `Planning lifecycle lock transport failure (retry ${attempt}/${MAX_RECOVERY_RETRIES} in ${delay}): ${errorMessage}`, undefined, deps.runContextFor(task.id));
        ...
        */
        ...
        try {
          ...
          if (deps.depAborted.has(task.id)) {
            ...
            await deps.store.logEntry(task.id, `Waiting for shared workspace resource: ${errorMessage}`, undefined, deps.runContextFor(task.id));
            throw err;
