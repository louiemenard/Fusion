...
async function writeWorkflowStepResultPatch(
  deps: Pick<ExecuteWorkflowGraphDeps, "store" | "getRunContextFor" | "runContextFor">,
  taskId: string,
  compute: (current: Task) => WorkflowStepResultPatch | null,
...
type PersistWorkflowStepResultDeps = Pick<ExecuteWorkflowGraphDeps, "store" | "getRunContextFor" | "runContextFor" | "readTaskArtifact">
    ...
    try {
      ...
      */
      ...
      const runner = new WorkflowGraphTaskRunner({
        ...
        `store.updateTask(taskId, {workflowStepResults}, runContextForTotal(deps.getRunContextFor, taskId))` path (task id is `taskId`, not `{workflowStepResults}`). Fail-soft: degrade to a
