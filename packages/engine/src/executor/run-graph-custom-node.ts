...
export async function runPlanReviewDependencyGate(
    ...
    for (const repository of input.workspaceConfig.repos) {
      ...
      if (!path || !existsSync(path)) {
        await input.store.logEntry(
          ...
          input.runContextFor(input.task.id),
        );
        continue;
      }
  ...
  for (const target of targets) {
    try {
      const readiness = await ensureWorktreeDependencies({
        ...
        store: input.store,
        runContext: input.runContextFor(input.task.id),
  ...
  await input.store.logEntry(input.task.id, "Plan Review blocked by worktree dependency readiness", output, input.runContextFor(input.task.id));
  return {
    outcome: "failure",
    value: "REVISE",
    contextPatch: {
    ...
    */
    if (isPlanReviewNode) {
      const dependencyGate = await runPlanReviewDependencyGate({
        ...
        settings,
        workspaceConfig,
        runContextFor: deps.runContextFor,
        worktreePath,
        store: deps.store,
    ...
    if (isDeterministicVerificationGate) {
      return runDeterministicVerificationGate({ store: deps.store, getRunContextFor: deps.getRunContextFor, runContextFor: deps.runContextFor }, node, executionTarget, settings, worktreePath);
    }
        ...
        */
        ...
        if (reviewSuperseded) {
          ...
          await deps.store.logEntry(
            ...
            deps.runContextFor(workspaceReviewTarget.id),
          );
          aggregate = {
            verdict: "UNAVAILABLE",
            retryable: true,
