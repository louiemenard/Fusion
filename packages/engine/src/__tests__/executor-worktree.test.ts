...
describe("TaskExecutor worktree recovery", () => {
  ...
  it("normalizes an out-of-root branch-conflict reclaim before persisting it", async () => {
    ...
    /*
    FNXC:Identity 2026-08-24-02:51:
    Restack of identity/3-engine-src into 4/5: keep the 3/5 provenance patch (branch +
    branchWriteOrigin from #3507) AND the 4/5 mutation-context matcher. Dropping either
    would let an unattributed reclaim or an engine-stamped operator branch pass.
    */
    expect(store.updateTask).toHaveBeenCalledWith(
      "FN-8400",
      expect.objectContaining({
